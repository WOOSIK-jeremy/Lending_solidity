...
contract Testx is Test {
    ...
    function testBorrowWithInSufficientCollateralAfterRepaymentFails() external {
        ...
        {
            (bool success,) = address(lending).call(
                abi.encodeWithSelector(UpsideAcademyLending.borrow.selector, address(usdc), 1000 ether) // 1000 , 3000
            );
            assertTrue(success);
            (success,) = address(lending).call(
                abi.encodeWithSelector(UpsideAcademyLending.borrow.selector, address(usdc), 1000 ether) // 2000, 2000
            );
            assertTrue(success);
            ...
            (success,) = address(lending).call(
                abi.encodeWithSelector(UpsideAcademyLending.repay.selector, address(usdc), 1000 ether) // 1000, 3000
            );
            assertTrue(success);

            (success,) = address(lending).call(
                abi.encodeWithSelector(UpsideAcademyLending.borrow.selector, address(usdc), 1000 ether) // 2000?? 1500 x ,
            );
            assertFalse(success);
    ...
    function testWithdrawYieldSucceeds() external {
        ...
        supplyUSDCDepositUser1(); // 100000000 user1 usdc
        supplySmallEtherDepositUser2(); // 1ether user2
